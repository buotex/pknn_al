...
local file = io.input("/home/bxu/data/letters/letter-recognition.data")
local t = {}
while true do
...
for i = 1, #t do
  labels.data[labels:index(i - 1,0)] = string.byte(t[i][1]) - 64
  for j = 2, #t[i] do
    data.data[data:index(i - 1,j - 2)] = tonumber(t[i][j])
  end
end
--data:view(0, "all"):print()
local classes = Matrix.range(1,27)
local numClasses = 26
local classMapping = Matrix.range(-1,26)
